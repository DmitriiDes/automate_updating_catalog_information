...
def get_additional_info(source_directory, file_extension):
    ...
    for description_path in description_path_list:
        product_description_lines = convert_txt_description_into_lines(
                                  description_path)
        additional_info += "name: {} <br />weight: {}<br /><br />".format(product_description_lines[0], product_description_lines[1])
...
def main():
    ...
    reports.generate_report(generated_report_path, report_name, title, additional_info)
    #Email related data
