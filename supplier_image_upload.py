...
def get_full_path_list_of_files(sourse_directory, file_extension):
    """
    Returns the list of full paths to the files with specified file_extension
    from the sourse_directory
    """
    full_path_list = []
    file_list = get_list_of_files_from_directory(sourse_directory,
                                                 file_extension)
    for file_name in file_list:
        full_path = os.path.join(sourse_directory, file_name)
...
def upload_all_from_the_sourse_directory(sourse_directory,
                                         file_extension, server_url):
    """
    Uploads all files with specified file_extension from a specified
    sourse_directory to a server
    """
    full_path_list = get_full_path_list_of_files(sourse_directory,
                                                 file_extension)
